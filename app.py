...
def _save_media_file(file_bytes: bytes, original_name: str) -> str:
    """Append the new upload to existing Media data.
    ...
    """
    # Parse the new upload first so we fail fast on a bad file.
    try:
    ...
    # Serialise the merged dataframe back to xlsx bytes.
    buf = io.BytesIO()
    ...
    meta = {
        ...
        "rows_added": int(len(df_combined) - (0 if not existing else len(df_old))),
    }
    if SB_ENABLED:
        ...
        (MEDIA_DIR / Path(remote_path).name).write_bytes(out_bytes)
...
def render_media() -> None:
    ...
    with st.expander("⬆️  Upload / append Plurimedia data", expanded=False):
        with st.form("upload_media", clear_on_submit=True):
            up = st.file_uploader(
                "Excel file (.xlsx) — appended to the existing data. "
                "Columns must match: Annonceur, Media, Support_1, Tarif Final, GRP "
                "(plus any extra columns already present, e.g. Catégorie).",
                type=["xlsx", "xls"],
                key="upl_media",
            )
            submitted = st.form_submit_button(
            ...
            if submitted:
                if up is None:
                    ...
                    try:
                        ...
                        st.session_state[flash_key] = (
                            "ok",
                            f"Plurimedia data appended{detail}.",
                        )
                    except Exception as exc:  # pragma: no cover
